...
class V2RayProvider with ChangeNotifier, WidgetsBindingObserver {
  ...
  static const platform = MethodChannel('id.web.idm.pvc/vpn_control');
  ...
  Future<void> _initialize() async {
    ...
    try {
      ...
      // If no saved configs exist (first launch or cleared storage), load the
      // bundled asset immediately so the app has servers available offline.
      // Then kick off a background refresh from the GitHub raw URL.
      if (_configs.isEmpty && _subscriptions.isEmpty) {
        debugPrint('No stored configs — loading bundled fallback...');
        final bundled = await _serverService.loadBundledConfigs();
        if (bundled.isNotEmpty) {
          _configs = bundled;
          await _v2rayService.saveConfigs(_configs);
          debugPrint('Loaded ${_configs.length} bundled configs');
          notifyListeners();
        }
        // Background refresh from GitHub (non-blocking)
        _refreshDefaultServersInBackground();
      }
  ...
  Future<void> _enhancedSyncWithVpnServiceState() async {
    try {
      debugPrint('Enhanced synchronization with VPN service state...');
      ...
      if (isActuallyConnected) {
        debugPrint('VPN is actually running, synchronizing config states...');
        ...
        if (activeConfigFromService != null) {
          ...
          if (!configFound) {
            debugPrint('No matching config found in list for active VPN connection');
          ...
          for (var config in _configs) {
            try {
              ...
              debugPrint('Marked config as connected by default: ${config.remark}');
              break;
            } catch (e) {
              debugPrint('Error checking config ${config.remark}: $e');
            }
          }
        ...
        debugPrint('VPN is not actually connected, clearing connection states');
      ...
      await _v2rayService.saveConfigs(_configs);
      debugPrint('VPN service state synchronization completed');
    } catch (e) {
      debugPrint('Error in enhanced synchronization with VPN service state: $e');
  ...
  /// Silently refresh default servers from GitHub in the background.
  /// Replaces non-subscription configs without interrupting the user.
  void _refreshDefaultServersInBackground() {
    _serverService.fetchServersWithFallback().then((fresh) async {
      if (fresh.isEmpty) return;
      // Keep subscription configs, replace default ones
      final subIds = <String>{};
      for (var sub in _subscriptions) {
        subIds.addAll(sub.configIds);
      }
      final subConfigs = _configs.where((c) => subIds.contains(c.id)).toList();
      _configs = [...subConfigs, ...fresh];
      await _v2rayService.saveConfigs(_configs);
      debugPrint('Background refresh: replaced defaults with ${fresh.length} fresh configs');
      notifyListeners();
    }).catchError((e) {
      debugPrint('Background refresh error (non-fatal): $e');
    });
  }
  ...
  /// Refresh default (non-subscription) servers from GitHub raw URL.
  /// Falls back to bundled asset if offline. Safe to call from UI.
  Future<void> refreshDefaultServers() async {
    if (_isLoadingServers) return;
    _isLoadingServers = true;
    _errorMessage = '';
    notifyListeners();
    try {
      final fresh = await _serverService.fetchServersWithFallback();
      if (fresh.isNotEmpty) {
        final subIds = <String>{};
        for (var sub in _subscriptions) {
          subIds.addAll(sub.configIds);
        ...
        _configs = [...subConfigs, ...fresh];
        await _v2rayService.saveConfigs(_configs);
        debugPrint('refreshDefaultServers: ${fresh.length} configs loaded');
      }
    } catch (e) {
      _setError('Failed to refresh servers: $e');
    } finally {
      _isLoadingServers = false;
      notifyListeners();
    }
